...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      #Select Household Language
      radioButtons(inputId = "hhl_corr",
                   label = tags$b("Household Language"),
                   choiceNames = list("All", 
                                      "English only", 
                                      "Spanish", 
                                      "Other"),
                   choiceValues = list(HHLvals,
                                       HHLvals["1"],
                                       HHLvals["2"],
                                       HHLvals[c("0", "3", "4", "5")])
                   ),
      
      # Select SNAP Recipient
      radioButtons(inputId = "fs_corr",
                   label = tags$b("SNAP Recipient"),
                   choiceNames = list("All", 
                                      "Yes", 
                                      "No"),
                   choiceValues = list(FSvals,
                                       FSvals["1"],
                                       FSvals["2"])
      ),
      ...
      radioButtons(inputId = "schl_corr",
                   label = tags$b("Educational attainment"),
                   choiceNames = list("All", 
                                      "High School not Completed", 
                                      "High School or GED", 
                                      "College Degree"),
                   choiceValues = list(SCHLvals,
                                       SCHLvals[c("0", "01", "02", "03", "04", "05", "06", "07", "08", "09"
                                                  ,"10", "11", "12", "13", "14", "15")],
      ...
      # Slider widget to select sample size
...
server <- function(input, output, session) {
    ...
    # #Use the code below to validate that data exists, and then create the appropriate scatter plot
    ...
    output$scatterplot <- renderPlot({
      ...
      ggplot(data = sample_corr$corr_data, 
             aes_string(x = isolate(input$corr_x), y = isolate(input$corr_y))) +
        geom_point()
    })
